...
def get_username(text):
    for line in text.splitlines():
        if "##" in line:
            ...
            # Başta kalan >, ›, - veya boşlukları sil (Hangi sembol gelirse gelsin atlar)
...
def parse_tiktok_message(text, chat_title):
    ...
    clean_lines = []
    for line in text.splitlines():
        # Tüm çöp reklam linklerini temizle
        if any(bad in line for bad in ["dichvu321", "junb.io.vn", "box-countdown", "http"]):
            continue
        # Tek başına kalan gereksiz işaretleri atla
        if line.strip() in [">", "=", "-", ""]:
            continue
    ...
    if username:
        # Arapça ve bozuk sembolleri tıklandığında sorun çıkarmayacak URL yapısına kodlar
