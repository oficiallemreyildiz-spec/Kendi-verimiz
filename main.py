...
def db_stats():
    try:
        ...
        goody = db.execute(
            """
            SELECT COUNT(*) FROM radar_history WHERE event_type='GOODY BAG'
            """
        ).fetchone()[0]
        chest = db.execute(
            """
            SELECT COUNT(*) FROM radar_history WHERE event_type='CHEST'
            """
        ).fetchone()[0]
        return {
            "total": total,
...
def db_load_recent():
    try:
        ...
        for row in reversed(rows):
            ...
            target = (
                LIVE_GOODY_BAGS if d["type"] == "GOODY BAG" else LIVE_CHESTS
            )
            if d["room"]:
                target[d["room"]] = d
...
def get_alarms(d):
    ...
    if (
        ...
        alarms.append({
            ...
            "title": "🚨 COIN ALARMI",
            "key": (
                event_key + "|COIN|" + str(COIN_ALARM_LIMIT)
            )
        })
    if (
        ...
        alarms.append({
            ...
            "key": (
                event_key + "|PEOPLE|" + str(PEOPLE_ALARM_LIMIT)
            )
        })
    return alarms
...
.go { display:block; text-align:center; color:#fff; text-decoration:none; background:#d71950; padding:11px; margin-top:9px; border-radius:9px; font-size:12px; font-weight:1000; }
...
@media(max-width:700px){
  ...
  .max-card { padding:8px; border-radius:9px; }
...
async def start_http():
    ...
    for path in [
        ...
        app.router.add_options(
            path,
            lambda request: web.Response(status=204)
        )
