...
SITE_BOT_TOKEN = os.environ.get("SITE_BOT_TOKEN", BOT_TOKEN)
...
def token_from_event(e):
    ...
    try:
        ...
        print("[TOKEN ENTITY TEXT]", repr(e))
...
def add(d):
    ...
    if room in target:
        old = target[room]
        if int(time.time()) - safe_int(old.get("detected_at")) < 5:
            return False
...
def alarm_exists(alarm_key):
    try:
        row = db.execute(
            "SELECT 1 FROM alarm_history WHERE alarm_key=? LIMIT 1",
            (alarm_key,)
        ).fetchone()
        return row is not None
    except Exception as e:
...
def save_alarm(alarm_key, event_key, alarm_type):
    try:
        db.execute(
            ...
            (alarm_key, event_key, alarm_type, created_at)
            VALUES (?, ?, ?, ?)
            """,
...
@keyframes in{0%{opacity:.3;transform:translateY(-10px) scale(.97);}40%{box-shadow:0 0 35px #9d51ff77;}100%{opacity:1;transform:none;}}
@media(max-width:700px){body{padding:5px;}.head{padding:8px 3px 12px;}.title{font-size:36px;}.sub{font-size:12px;margin-top:9px;}.status{font-size:10px;padding:8px 12px;margin-top:9px;border-width:1px;}.grid{gap:5px;}.maxbox{padding:7px;border-radius:11px;margin-bottom:6px;}.max-title{font-size:11px;margin-bottom:7px;}.max-card{padding:8px;border-radius:99px;}.maxbox.g .max-card,.maxbox.c .max-card{border-left-width:4px;}.max-user{margin-bottom:7px;gap:5px;}.max-user-name{font-size:11px;}.max-coin{font-size:15px;}.max-stats{gap:4px;}.max-stat{padding:6px;font-size:6px;}.max-stat b{font-size:11px;}.max-live{padding:8px;margin-top:6px;font-size:8px;}.max-link{font-size:6px;margin-top:5px;}.panel{padding:6px;border-radius:11px;}.pnrow{padding:3px 2px 7px;}.pn{font-size:12px;}.cnt{font-size:9px;padding:4px 6px;}.card{padding:8px;margin-bottom:5px;border-radius:9px;}.g .card,.c .card{border-left-width:4px;}.ur{margin-bottom:7px;gap:5px;}.user{font-size:11px;}.rank{font-size:7px;}.badge{font-size:7px;padding:4px 6px;}.ig{gap:4px;}.info{padding:6px;font-size:6px;}.info b{font-size:11px;margin-top:2px;}.go{padding:8px;margin-top:6px;font-size:8px;}.empty{padding:16px;font-size:8px;}.foot{font-size:7px;padding:9px;}}
@media(max-width:390px){.title{font-size:31px;}.sub{font-size:10px;}.status{font-size:8px;}.max-title{font-size:10px;}.max-user-name{font-size:10px;}.max-coin{font-size:13px;}.max-stat{font-size:5px;padding:5px;}.max-stat b{font-size:10px;}.pn{font-size:10px;}.user{font-size:10px;}.info{font-size:5px;padding:5px;}.info b{font-size:10px;}.go{font-size:7px;}}
</style>
</head>
<body>
<div class="wrap">
...
e.innerHTML=`
<div class="max-card">
<div class="max-user"><div class="max-user-name">${icon} ${esc(max.username)}</div><div class="max-coin">🪙 ${esc(max.coins)}</div></div>
<div class="max-stats">
...
arr.forEach(x=>{const k=key(x);if(first){seen.add(k);}else if(!seen.has(k)){seen.add(k);ns.add(k);}});
e.innerHTML=arr.map((x,i)=>{
const k=key(x);
const fresh=ns.has(k);
return `
<div class="card ${fresh?"new":""}">
<div class="ur"><div class="user">${icon} ${esc(x.username)}</div>${fresh?'<div class="badge">⚡ YENİ</div>':`<div class="rank">#${i+1}</div>`}</div>
<div class="ig">
...
data={goody_bags:Array.isArray(d.goody_bags)?d.goody_bags:[],chests:Array.isArray(d.chests)?d.chests:[]};
const s=document.getElementById("status");
s.className="status";s.textContent="🟢 RADAR AKTİF • CANLI VERİ";
render();first=false;
}catch(e){
const s=document.getElementById("status");
s.className="status";s.textContent="🔴 VERİ BAĞLANTISI HATASI";
}
}
    ...
    try:
        ...
        for alarm in alarms:
            ...
            if save_alarm(alarm["key"], event_key, alarm["type"]):
                print("[ALARM]", alarm["title"], "|", d["username"], "| COIN:", d["coins"], "| KİŞİ:", d["people"])
